...
school_assessment <- collapse %>%
    ...
    mutate(grade = if_else(grade == 0, "Missing Grade", as.character(grade)),
        ...
        pct_on_track = if_else(valid_tests != 0, round(1000 * n_on_track/(valid_tests*10), 1), NA_real_),
        pct_mastered = if_else(valid_tests != 0, round(1000 * n_mastered/(valid_tests*10), 1), NA_real_),
        pct_below = if_else(valid_tests != 0, round(100 - pct_approaching - pct_on_track - pct_mastered, 1), NA_real_),
        pct_on_mastered = if_else(valid_tests != 0, round(100 * (n_on_track + n_mastered)/(valid_tests*10), 1), NA_real_),
